        ...
        public async Task<ScenarioResult> HandleMessageAsync(ITelegramBotClient botClient, ScenarioContext context, Message message, CancellationToken ct)
        {
            ...
            switch (context.CurrentStep)
            {
                ...
                case "scheduleActions":
                    ...
                    callbackData.Clear();
                    ToDoUser user = await _userService.GetUserAsync(Int32.Parse(context.Data["scheduleId"].ToString()), ct);
                    ...
                    await botClient.EditMessageText(callbackMessage.Chat, callbackMessage.MessageId, "Выберите смену для удаления.", replyMarkup: PageBuilder.BuildPagedButtons(callbackData, PagedListCallbackDto.FromString("showShiftPageNext||0")), cancellationToken: ct);
                ...
                case "selectShift":
                    ...
                    {
                        user = await _userService.GetUserAsync(Int32.Parse(context.Data["scheduleId"].ToString()), ct);
                    ...
                    {
                        // Если мы получаем в колбеке данные о кнопках смены страниц, то попадаем сюда и меняем страницу, а дальше сохраняем шаг тем же.
                        await botClient.EditMessageText(callbackMessage.Chat, callbackMessage.MessageId, "Выберите смену для удаления.", replyMarkup: PageBuilder.BuildPagedButtons(callbackData, PagedListCallbackDto.FromString(context.Data["Callback"].ToString())), cancellationToken: ct);
                        return ScenarioResult.Transition;
                    }
