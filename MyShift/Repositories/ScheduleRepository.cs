...
namespace MyShift.Repositories
{
    internal class ScheduleRepository : IScheduleRepository
    {
        ...
        {
            using var context = _dbFactory.CreateDbContext();
            await context.AddAsync(templ_schedule);
            await context.SaveChangesAsync(ct);
        }
        ...
        {
            using var context = _dbFactory.CreateDbContext();
            return await context.Schedule_Templates.ToListAsync();
        }
        ...
        {
            using var context = _dbFactory.CreateDbContext();
            return await context.Schedule_Templates.FirstOrDefaultAsync(tmp => tmp.Id == id, cancellationToken: ct);
        }
        ...
        {
            using var context = _dbFactory.CreateDbContext();
            UserSchedule schedule = await context.UserSchedules.FirstOrDefaultAsync(sch => sch.Id == scheduleId);
        ...
        {
            using var context = _dbFactory.CreateDbContext();
            Shift shift = await context.Shifts.FirstOrDefaultAsync(s => s.Id == shiftId);
            shift.Status = false;
            await context.SaveChangesAsync(ct);
        }
